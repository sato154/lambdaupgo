...
func (lp LProject) UploadLambda(name string) error {
	...
	fmt.Println(lp.Region + "1")
	fl, err := NewFunctionList(lp.Region)
	if err != nil {
		return err
	}

	if fl.HasFunction(lamname) {
		resp, err := run("aws", "lambda", "update-function-code", "--function-name", lamname, "--s3-bucket", lp.Bucket, "--s3-key", lamname+".zip", "--region", lp.Region)
		if err != nil {
			return err
		}
